            ...
            <div class="message ${isSent ? 'sent' : 'received'}">
                ...
                <div class="message-text">
                    ...
                    <div class="message-status">
                        ...
                        ${msg.isEdited ? ' <span class="edited-tag" style="font-size:9px; opacity:0.6;">(edited)</span>' : ''}
                    </div>
                    ...
                    <div id="options-${msg.id}" class="message-options-dropdown" style="display: none;">
                        ${isSent ? `<button onclick="editMessageUI(${msg.id})"><i class="fa-solid fa-pen"></i> Edit</button>` : ''}
                        ${isSent ? `<button onclick="confirmDeleteMessage(${msg.id})" class="delete"><i class="fa-solid fa-trash"></i> Delete</button>` : ''}
                        <button onclick="copyMessageText(${msg.id})"><i class="fa-solid fa-copy"></i> Copy</button>
                    </div>
                </div>
            </div>
        `;
...
// Message Editing & Deletion Logic
...
function toggleMessageOptions(event, messageId) {
    ...
    const wasVisible = dropdown && dropdown.style.display === 'flex';
    
    // Close all other dropdowns
    document.querySelectorAll('.message-options-dropdown').forEach(d => d.style.display = 'none');
    
    if (dropdown && !wasVisible) {
...
function copyMessageText(messageId) {
    ...
    if (pEl) {
        ...
        navigator.clipboard.writeText(text).then(() => {
            if (typeof showMessage === 'function') showMessage('Copied to clipboard!', 'success');
        });
    }
}
...
async function confirmDeleteMessage(messageId) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (typeof showMessage === 'function') showMessage('Message deleted', 'success');
            loadConversations();
        } else {
            showMessage('Could not delete message. Please try again.', 'error');
        }
    } catch (e) {
...
function cancelEdit(messageId) {
    ...
    if (messageEl) {
        const bodyEl = messageEl.querySelector('.message-body');
        if (bodyEl) bodyEl.innerHTML = `<p style="margin:0;">${originalContent}</p>`;
        const optBtn = messageEl.querySelector('.message-options-btn');
...
async function saveEdit(messageId) {
    ...
    try {
        ...
        if (response.ok) {
            editingMessageId = null;
            const bodyEl = messageEl.querySelector('.message-body');
            if (bodyEl) bodyEl.innerHTML = `<p style="margin:0;">${newContent}</p>`;
            ...
            if (statusEl && !statusEl.querySelector('.edited-tag')) {
                statusEl.insertAdjacentHTML('beforeend', ' <span class="edited-tag" style="font-size:9px; opacity:0.6;">(edited)</span>');
            }
